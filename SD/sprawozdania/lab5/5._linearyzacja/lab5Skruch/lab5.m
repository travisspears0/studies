...
WPC = [cos(p); sin(p)]; %tworzymy macierz warunków pocz¹tkowych

dt = 10^-4;
T = 2;
   
%wykonujemy pêtle w której rysujemy portrety fazowe dla macierzy warunków pocz¹tkowych
...
for i=1:length(p) 
    ...
    plot(XRK(:,1), XRK(:,2));
    %opisujemy wykres
    title('Wykres zaleznosci x1 x2 dla s. zlinearyzowanego dla pkt równowagi (0,0)');           
    xlabel('x1');   % os x
    ylabel('x2');   % os y
end;
hold off;
...
for i=1:length(p) 
    ...
    plot(XRK(:,1), XRK(:,2));
    %opisujemy wykres
    title('Wykres zaleznosci x1 x2 dla s. zlinearyzowanego dla pkt równowagi (-2/3,0)');           
    xlabel('x1');   % os x
    ylabel('x2');   % os y
end;
hold off;